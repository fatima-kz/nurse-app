...
export default function DashboardContent() { // Renamed from Dashboard to DashboardContent
  const [user, setUser] = useState<UserProfile | null>(null);
  const [testHistory, setTestHistory] = useState<TestResultDisplay[]>([]);
  const [loading, setLoading] = useState(true);
  const [upgradeStatus, setUpgradeStatus] = useState<string | null>(null);
  const searchParams = useSearchParams(); // This hook is now inside this client component
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    setLoading(false);
  }, []); // Dependencies for loadDashboardData

  useEffect(() => {
    loadDashboardData();

    // Check for upgrade status in URL params
    ...
    // Clear the upgrade status after showing it
    if (upgrade) {
      setTimeout(() => {
        setUpgradeStatus(null);
      }, 5000);
    }
  }, [searchParams, loadDashboardData]); // Include loadDashboardData as a dependency
                 ...
                 {upgradeStatus === 'success' && (
                     <div className="mb-6 bg-green-50 border border-green-200 rounded-lg p-4">
                         ...
                         <p className="text-green-700 text-sm mt-1">
                             Welcome to Premium! Your subscription is now active and you have unlimited access to all features.
                         </p>
                     </div>
                 )}
                 ...
                 {upgradeStatus === 'cancelled' && (
                     <div className="mb-6 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                         ...
                         <p className="text-yellow-700 text-sm mt-1">
                             No worries! You can upgrade to Premium anytime to unlock unlimited access.
                         </p>
                     </div>
                 )}
